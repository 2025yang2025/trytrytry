    ...
    try:
        ...
        if res.status_code == 200:
            for item in res.json():
                ...
                # 過濾邏輯：只留標準 4 位數純個股，排除 00 開頭之 ETF 與特殊證券
...
def get_ticker_df(full_df, ticker):
    """ 安全取得單一 Ticker 的 DataFrame """
    try:
        if isinstance(full_df.columns, pd.MultiIndex):
            if ticker in full_df.columns.levels[1]:
                return full_df.xs(ticker, axis=1, level=1)
            elif ticker in full_df.columns.levels[0]:
                return full_df[ticker]
        return pd.DataFrame()
    except Exception:
        return pd.DataFrame()
...
def calculate_macd(close_series, fast=12, slow=26, signal=9):
    ...
    macd_line = fast_ema - slow_ema  # DIF
    signal_line = macd_line.ewm(span=signal, adjust=False).mean()  # DEM
...
def check_macd_heading_to_zero_kd(df_tf, kd_threshold=20):
    ...
    try:
        ...
        macd_line, _, _ = calculate_macd(c_tf)
        if len(macd_line) < 2: return False, 0.0
...
def check_strategy_ma20_rebound(df_d, df_w, df_m):
    """ 精準拉回策略：月/週長線多頭 + 日K 20MA (月線) 有撐且 MACD 柱狀體翻紅 """
    try:
        # 1. 月 K 檢測
...
if __name__ == "__main__":
    ...
    full_df_daily = safe_download_yf(tech_scan_pool, period="1y", interval="1d", chunk_size=200)
    full_df_weekly = safe_download_yf(tech_scan_pool, period="2y", interval="1wk", chunk_size=200)
    full_df_monthly = safe_download_yf(tech_scan_pool, period="5y", interval="1mo", chunk_size=200)
    ...
    for ticker in tech_scan_pool:
        try:
            ...
            # 門檻：20日均量 >= 2500張 (Volume / 1000.0 >= 2500)
            ...
            df_m = get_ticker_df(full_df_monthly, ticker)
            ...
            # 【精準拉回策略：月/週趨勢多頭 + 日K 20MA (月線) 有撐且 MACD 柱狀體翻紅】
    ...
    # ⏳ 步驟 3: 下載精選個股之 30分K 與 60分K 資料
    # 優先選擇符合「策略三」或「策略十」的標的進行分 K 下載
    heavy_scan_pool = list(set(strat3_map.keys()) | set(strat10_map.keys()))
    final_heavy_pool = heavy_scan_pool[:120]
